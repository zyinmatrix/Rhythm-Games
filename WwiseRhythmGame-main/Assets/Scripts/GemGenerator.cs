...
public class GemGenerator : MonoBehaviour
{
    ...
    public FallingGemInput fallingGemQ, fallingGemW, fallingGemO, fallingGemP, fallingBombQ, fallingBombW, fallingBombO, fallingBombP;
    ...
    public void GenerateQBombCue()
    {
        //we need to instantiate the cue, set the desired player input accordingly, and then set the window timings
        GameObject newCue = Instantiate(fallingBombQ.cuePrefab, fallingBombQ.cueStartLocation.transform.position, Quaternion.identity);

        FallingGem fallingGem = newCue.GetComponent<FallingGem>();

        fallingGem.playerInput = fallingBombQ.playerInput;
        fallingGem.isBomb = true; // label the bomb

        SetGemTimings(fallingGem);
    }

    public void GenerateWBombCue()
    {
        //we need to instantiate the cue, set the desired player input accordingly, and then set the window timings
        GameObject newCue = Instantiate(fallingBombW.cuePrefab, fallingBombW.cueStartLocation.transform.position, Quaternion.identity);

        FallingGem fallingGem = newCue.GetComponent<FallingGem>();

        fallingGem.playerInput = fallingBombW.playerInput;
        fallingGem.isBomb = true;

        SetGemTimings(fallingGem);
    }

    public void GenerateOBombCue()
    {
        //we need to instantiate the cue, set the desired player input accordingly, and then set the window timings
        GameObject newCue = Instantiate(fallingBombO.cuePrefab, fallingBombO.cueStartLocation.transform.position, Quaternion.identity);

        FallingGem fallingGem = newCue.GetComponent<FallingGem>();

        fallingGem.playerInput = fallingBombO.playerInput;
        fallingGem.isBomb = true;

        SetGemTimings(fallingGem);
    }

    public void GeneratePBombCue()
    {
        //we need to instantiate the cue, set the desired player input accordingly, and then set the window timings
        GameObject newCue = Instantiate(fallingBombP.cuePrefab, fallingBombP.cueStartLocation.transform.position, Quaternion.identity);

        FallingGem fallingGem = newCue.GetComponent<FallingGem>();

        fallingGem.playerInput = fallingBombP.playerInput;
        fallingGem.isBomb = true;
